...
ALLOY_COMPONENTS = {
    'c1': 0.10, 'c2': 0.10, # Fe (итого 20%)
    'c3': 0.25, 'c4': 0.25, # Ni (итого 50%)
    'c5': 0.15, 'c6': 0.15  # Cr (итого 30%)
}

alloy_name = "Fe20Ni50Cr30"  # Для отображения в заголовке и инфо-блоке
...
def plot_alloy_total_dos(basename, e_range=None, dos_range=None, sigma_ev=0.06, font_size=12):
    try:
        ...
        plt.plot(energy_ev_rel, dos_up_p, label='Spin Up', color='royalblue', lw=2)
        plt.plot(energy_ev_rel, -dos_down_p, label='Spin Down', color='crimson', lw=2)
        ...
        plt.title(f'Density of States of Random Solid Solution {alloy_name}', fontsize=font_size+4, fontweight='bold')
...
plot_alloy_total_dos('DOS_Fe20Ni50Cr30_atom000001', e_range=[-10, 5], dos_range=[-25, 45], sigma_ev=0.06)