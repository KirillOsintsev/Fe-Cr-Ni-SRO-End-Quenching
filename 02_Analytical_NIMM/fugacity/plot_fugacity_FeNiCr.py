...
# --- Параметры ---
FOLDER_PATH = '/Users/osintsevkirill/Yandex.Disk.localized/Science/Postdoc_project/Fe-Cr-Ni-SRO-End-Quenching/02_Analytical_NIMM/fugacity/all_fugacity_results'  # Укажите путь к папке с вашими JSON файлами
# FOLDER_PATH = '/path/to/your/json/files' # Пример для Linux/macOS
# FOLDER_PATH = 'C:\\path\\to\\your\\json\\files' # Пример для Windows

FILE_SUFFIX = '_fugacity_results.txt'

TEMPERATURE = '1273'  # Температура ('100', '200', ..., '1200')
...
"""
TARGET_ALPHA = 'CrNi'  # Какую альфу отображать ('alpha_FeNi', 'alpha_NiCr', 'alpha_FeCr')
COLORBAR_MIN = 1.0  # Минимальное значение для цветовой шкалы
COLORBAR_MAX = 1.5 # Максимальное значение для цветовой шкалы
"""

"""
TARGET_ALPHA = 'CrFe'  # Какую альфу отображать ('alpha_FeNi', 'alpha_NiCr', 'alpha_FeCr')
COLORBAR_MIN = 0.5 # Минимальное значение для цветовой шкалы
COLORBAR_MAX = 1.5 # Максимальное значение для цветовой шкалы
"""

TARGET_ALPHA = 'FeNi'  # Какую альфу отображать ('alpha_FeNi', 'alpha_NiCr', 'alpha_FeCr')
COLORBAR_MIN = 0.5  # Минимальное значение для цветовой шкалы
COLORBAR_MAX = 1.5 # Максимальное значение для цветовой шкалы
...
if len(grid_a) > 0:
    # Разделяем сетку на значения внутри и вне диапазона 0.5 - 1.5
    in_range_mask = (grid_alpha_filtered >= COLORBAR_MIN) & (grid_alpha_filtered <= COLORBAR_MAX)
    out_range_mask = ~in_range_mask

    # Трейс 1: Значения ВНЕ диапазона (окрашиваем в нейтральный серый)
    if out_range_mask.any():
        fig.add_trace(go.Scatterternary(
            a=grid_a[out_range_mask],
            b=grid_b[out_range_mask],
            c=grid_c[out_range_mask],
            mode='markers',
            marker=dict(
                symbol='square',
                size=12,
                opacity=0.6,          # Немного прозрачности для "неважных" зон
                color='lightgrey',    # Нейтральный цвет
    ...
    # Трейс 2: Значения ВНУТРИ диапазона 
    if in_range_mask.any():
        fig.add_trace(go.Scatterternary(
            a=grid_a[in_range_mask],
            b=grid_b[in_range_mask],
            c=grid_c[in_range_mask],
            mode='markers',
            marker=dict(
                symbol='square',
                size=12,
                opacity=1.0,
                color=grid_alpha_filtered[in_range_mask],
                colorscale=CUSTOM_COLORSCALE,
                reversescale=REVERSE_COLORSCALE,
                cmin=COLORBAR_MIN,
                cmax=COLORBAR_MAX,
                line=dict(width=0.2, color='rgba(0,0,0,0)'),
                # ПЕРЕНОСИМ НАСТРОЙКИ ШКАЛЫ СЮДА:
                colorbar=dict(
                    title=dict(
                    ...
                    orientation='h',
                    x=0.5,
                    y=1.25, # ПОДНЯЛИ ВЫШЕ (было 1.1), чтобы не пересекаться с "Cr"
                    xanchor='center',
                    yanchor='bottom',
                    len=0.5,
...
fig.update_layout(
    ...
    ternary=dict(
        ...
        aaxis=dict(
            ...
            gridcolor='lightgrey',
            tickvals=[30, 40, 50],
            tickfont=dict(size=24, color="black") # <--- добавили color="black" сюда
        ),
        baxis=dict(
            title=dict(
                text='Fe',  
            ...
            gridcolor='lightgrey',
            tickvals=[30, 40, 50],
            tickfont=dict(size=24, color="black") # <--- и сюда
        ),
        caxis=dict(
            title=dict(
                text='Ni',  
    ...
    annotations=[
            dict(
                ...
                y=1.0,        # Высота над треугольником (меняйте от 0.9 до 1.05 для идеальной позиции)
                xref='paper',
                yref='paper',
                showarrow=False,
                font=dict(size=26, family="Arial", color="black"),
...
#             y=1.2,                  # Смещение вверх над графиком
