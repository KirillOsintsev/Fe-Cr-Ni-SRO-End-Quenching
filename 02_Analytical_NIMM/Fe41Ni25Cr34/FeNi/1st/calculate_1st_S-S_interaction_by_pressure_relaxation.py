...
def objective_function(lattice_parameter, template, verbose=False, file=None):
    ...
    if verbose:
        # If you still want any other LAMMPS command-line arguments for verbose mode, put them here.
        # But DO NOT include '-v path'.
        lmp_args = []
    else:
        # For silent mode, keep arguments that control LAMMPS output.
        lmp_args = ["-echo", "none", "-screen", "none"]

    lmp = lammps(comm=MPI.COMM_WORLD, cmdargs=lmp_args)
